...
const ProfileScreen = (props) => {
                ...
                .then(resp => {
                    resp.forEach(doc => {
                        ...
                        if (doc.data().LolAccount != null) { // !=
                            let source = selector.lolLeagueImageSelector(doc.data().LolAccount["SoloQueueRanked"]);
                            games.push({
                            ...
                            let source = selector.valorantImageSelector(doc.data().ValorantAccount["League"])
                            ...
                            let source = selector.apexImageSelector(doc.data().ApexAccount["League"])
                            games.push({
                                name: "Apex Legends",
    ...
    const renderItem = ({ item }) => {
        return (
            <ListItem containerStyle={{ backgroundColor: "#fff" }}>
                ...
                <ListItem.Content>
                    <ListItem.Title style={{ fontFamily: "Roboto-Medium" }}>{item.name}</ListItem.Title>
                    <ListItem.Subtitle>{item.subtitle}</ListItem.Subtitle>
                </ListItem.Content>
                <ListItem.Title>
                    {item.league == false
                        ? <Icon name="edit" type="font-awesome" size={35} onPress={() => _onPress(item.name)} />
    ...
    const renderChoice = (c) => {
        if (c == "Apex Legends") {
            return (
                <View style={{ height: 250, width: 350, backgroundColor: "#ffffff", borderRadius: 30, overflow: "hidden" }}>
                    <View style={{ flex: 1 }}>
                        ...
                        <View style={style.apexButtonView}>
                            <Icon name="times-circle" type="font-awesome" size={60} color="red" containerStyle={{ paddingBottom: 0, alignSelf: 'center' }} onPress={() => {
                            ...
                            <View style={{ alignSelf: 'center', backgroundColor: "red" }}>
                                <Button
                                    ...
                                    buttonStyle={{ backgroundColor: "green", width: 150, height: 50, justifyContent: "space-around" }}
                                    ...
                                    onPress = { () => {
                                        ApexData.forEach(elem => {
                                            if (elem.id === selectedID) {
                                                ...
                                                dispatch(profile_screen_league_set("Apex Legends" , elem.name))
    ...
    return (
        <View style={{ backgroundColor: "#fff", height: "100%", width: "100%" /* Maybe can write flex : 1 , instead of height : 100 and width : 100*/ }}>
            <Modal
                animationType="fade"
                transparent={true}
                visible={loading}>
...
export default connect(mapStateToProps, { profile_screen_stats_set, profile_screen_league_set })(ProfileScreen);